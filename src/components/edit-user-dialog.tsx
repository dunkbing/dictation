...
export function EditUserDialog({ user, open, onOpenChange }: EditUserDialogProps) {
  ...
  useEffect(() => {
    async function loadPermissions() {
      const res = await apiClient.roles.permissions.$get();
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (Number.parseInt(selectedRoleId) !== user.roleId) {
        const roleRes = await apiClient.users[":id"].role.$patch({
          param: { id: user.id },
          json: { roleId: Number.parseInt(selectedRoleId) },
        });
        if (!roleRes.ok) {
          const body = (await roleRes.json()) as { error?: string };
      ...
      const permRes = await apiClient.users[":id"].permissions.$patch({
        param: { id: user.id },
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <p className="text-xs text-muted-foreground mb-2">
                Add or remove additional permissions beyond the role. Checked items in{" "}
                <span className="text-muted-foreground opacity-50">gray</span> are from the role.
              </p>
                      ...
                      <div key={permission} className="flex items-center space-x-2">
                        ...
                        <label
                          ...
                          className={`text-sm font-medium leading-none peer-disabled:cursor-not-allowed ${
                            fromRole ? "text-muted-foreground opacity-50" : "cursor-pointer"
                          }`}
                        >
                          {permission}
                          {fromRole && <span className="ml-2 text-xs">(from role)</span>}
                        </label>
                      </div>
                    );
                  })}
