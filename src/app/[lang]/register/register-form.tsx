...
export default function RegisterForm({ dict, lang }: { dict: Dictionary; lang: Locale }) {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const res = await apiClient.auth.register.$post({ json: value });
      if (!res.ok) {
        ...
        >
          ...
          <CardFooter className="flex flex-col space-y-4">
            ...
            <p className="text-center text-sm text-muted-foreground">
              ...
              <Link href={`/${lang}/login`} className="font-semibold hover:underline">
